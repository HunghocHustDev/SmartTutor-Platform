...
from app.schemas.entities import InvoicePeriodResponse, TuitionSummaryResponse
...
@router.get("/{class_id}/invoice-period", response_model=InvoicePeriodResponse)
def get_class_invoice_period(
    class_id: int,
    actor: CurrentActor = Depends(get_current_actor),
    db: Session = Depends(get_db),
):
    study_class = service.get_class_or_404(db, class_id)
    ensure_class_access(actor, study_class)
    return service.get_invoice_period(db, class_id)
